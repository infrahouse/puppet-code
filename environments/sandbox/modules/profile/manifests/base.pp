...
class profile::base () {
  ...
  # In the init stage: the man-db trigger has to be disarmed before ANY Package
  # resource in stage main, because a marker removed halfway through the catalog
  # does not refund the ~140s each already-evaluated Package paid. See the class
